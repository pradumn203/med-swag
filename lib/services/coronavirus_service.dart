...
import 'package:swag_health/models/Districts.dart';
...
class CoronavirusService {
  ...
  Future<CoronavirusData> getLocationDataFromCountryCode(
    ...
    print('live/country/${(Countries().resolveName(countryCode).toLowerCase())}/status/confirmed/date/${(formattedDate.toString())}T${(time.toString())}');

    final response =
    await http.get(baseUrl + 'live/country/${(Countries().resolveName(countryCode).toLowerCase())}/status/confirmed/date/${(formattedDate.toString())}T${(time.toString())}Z');
